...
"${engine_binary}" --list-interfaces >/dev/null || fail "moteur non exécutable"
...
if ${runtime_check}; then
    ...
    if [[ -e "/Library/Audio/Plug-Ins/HAL/AESBridge.driver" ]]; then
        ...
        "${smoke_test}" "${stage_driver}/Contents/MacOS/AESBridge" || fail "test HAL/RTP bout en bout échoué"
    else
