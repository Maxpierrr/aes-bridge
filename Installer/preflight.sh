...
checker_binary="${manager}/Contents/Resources/AESBridgeCoreAudioCheck"
...
[[ -x "${checker_binary}" ]] || fail "diagnostic Core Audio embarqué absent"
...
check_architecture "${checker_binary}" "diagnostic Core Audio"
...
/bin/chmod -R u+rwX,go+rX,go-w "${stage_driver}" "${stage_manager}"
...
/usr/bin/codesign --force --sign - "${stage_manager}/Contents/Resources/AESBridgeCoreAudioCheck"
...
[[ "$(/usr/bin/stat -f '%Lp' "${stage_driver}/Contents/Info.plist")" == "644" ]] || fail "permissions du plist pilote incorrectes"
[[ "$(/usr/bin/stat -f '%Lp' "${stage_manager}/Contents/Info.plist")" == "644" ]] || fail "permissions du plist application incorrectes"
