...
void LXToolIOHandler::OnReadClientInput(const std::shared_ptr<aspl::Client>&,
    ...
    bool underrun = false;
    for (std::size_t ch = 0; ch < kVirtualChannels; ++ch) {
        ...
        interleaveChannel<Float32>(std::span(channel).first(frameCount), ch, kVirtualChannels,
            std::span(output, static_cast<std::size_t>(frameCount) * kVirtualChannels));
    }
...
void LXToolIOHandler::OnWriteClientOutput(const std::shared_ptr<aspl::Client>&,
    ...
    bool overrun = false;
    for (std::size_t ch = 0; ch < kVirtualChannels; ++ch) {
        deinterleaveChannel<Float32>(std::span(frames, static_cast<std::size_t>(frameCount) * kVirtualChannels),
            ch, kVirtualChannels, std::span(channel).first(frameCount));
