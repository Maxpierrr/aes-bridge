...
- a named shared-memory audio layout (`Local\\AESBridge.Audio.v2`) matching the
  eight-channel engine/endpoint boundary;
...
8 capture and 8 render channels at 48 kHz and exchange only preallocated blocks
...
Windows audio endpoint (future WDK project)
    ↕ named mapping + lock-free per-channel rings
AES Bridge Windows backend
    ↕ RTP L24 + SAP/SDP + PTP state
AES67 network
```
